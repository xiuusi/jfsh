...
> 相比上游的改动：
> - **媒体库浏览**：新增 "Libraries" 标签页，可直接浏览 Jellyfin 媒体库。（由 minimax-m2.7 实现）
> - **季（Season）导航**：支持按季浏览电视剧（剧集 → 季 → 单集），并展示季信息。（由 deepseek-v4-pro 实现）
> - **手动跳过片段**：在 mpv 播放窗口中按 `Ctrl+s` 可跳过当前片段（片头、片尾等），不受 `skip_segments` 自动跳过配置的限制。（由 opus 4.6 实现）
> - **媒体信息显示**：在副标题行显示电影和剧集的分辨率、编码、HDR规格等信息。（由 minimax-m2.7 实现）
> - **类型标签显示**：在副标题行显示电影、剧集、视频的类型（Genre）。（由 deepseek-v4-pro 实现）
...
- 追踪播放进度并同步到 Jellyfin
- 自动及手动跳过片段（片头、片尾等）
- **媒体库浏览**：通过 Libraries 标签页直接浏览媒体库
- **季导航**：支持按季浏览电视剧（剧集 → 季 → 单集）
- **媒体信息显示**：显示电影和剧集的分辨率、编码、HDR规格等信息
- **类型标签显示**：显示电影、剧集、视频的类型标签
- 纯键盘操作，无需鼠标
...
4. **导航标签页**
   ...
   - **Resume**：继续观看（上次未看完的媒体）
   - **Next Up**：接下来播放（电视剧下一集）
   - **Recently Added**：最近添加
   - **Libraries**：媒体库列表
   - **Search**：搜索
...
5. **浏览媒体库**
   ...
   - 切换到 **Libraries** 标签页查看所有媒体库
   - 选择一个媒体库按 **Enter** 进入，可浏览该库中的所有内容
   - 对于剧集，选中后可查看**季列表**，再选择某季查看单集
   - 选择具体条目（如电影、单集）按 **Enter** 播放
   - 按 **Esc** 或 **Backspace** 返回上级目录
